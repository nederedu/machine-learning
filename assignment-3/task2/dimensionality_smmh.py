...
def preprocess_data(df):
    # Convert 'Timestamp' column to datetime and then to numeric (timestamp)
    df['Timestamp'] = pd.to_datetime(df['Timestamp'], errors='coerce').map(pd.Timestamp.timestamp)
    
    # Convert specific categorical columns to numeric using one-hot encoding
...
def main():
    ...
    n_components_rp = min(data_scaled.shape[1], 12)  # Adjust based on your needs
    n_components_pca = min(data_scaled.shape[1], 3)  # Adjust based on your needs
    n_components_ica = min(data_scaled.shape[1], 7)  # Adjust based on your needs
